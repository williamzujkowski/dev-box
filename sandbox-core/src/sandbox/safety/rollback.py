...
from ..utils.stub_modules import CompressionUtils
from ..utils.stub_modules import DataEncryption
from ..utils.stub_modules import IntegrityChecker
...
class RollbackManager:
    ...
    def __init__(self, config):
        ...
        self.encryption = (
            DataEncryption()
            if config.safety_constraints.get("encrypt_snapshots")
            else None
        )
        ...
        self.compression_enabled = config.safety_constraints.get(
            "compress_snapshots", True
        )
        self.incremental_enabled = config.safety_constraints.get(
            "incremental_snapshots", True
        )
        self.auto_cleanup_hours = config.safety_constraints.get(
    ...
    async def create_snapshot(
        ...
        try:
            ...
            compression_ratio = (
                (1 - size_bytes / uncompressed_size) if uncompressed_size > 0 else 0
            )
            ...
            self.logger.info(
                f"Snapshot created successfully: {snapshot_id} ({size_bytes} bytes, {file_count} files)"
            )
    ...
    async def restore_snapshot(
        self, snapshot_id: str, rollback_type: str = "full", backup_current: bool = True
    ) -> RollbackResult:
        """
        ...
        try:
            ...
            if backup_current:
                ...
                if backup_snapshot_id:
                    ...
                    self.logger.info(
                        f"Created pre-rollback backup: {backup_snapshot_id}"
                    )
                else:
            ...
            self.logger.info(
                f"Rollback completed successfully: {files_restored} files restored in {result.rollback_duration:.2f}s"
            )
    ...
    async def cleanup_old_snapshots(self, max_age_hours: Optional[int] = None) -> int:
        ...
        try:
            ...
            for snapshot_id, metadata in self.snapshots_metadata.items():
                if (
                    metadata.timestamp < cutoff_time
                    and metadata.snapshot_type == "automatic"
                    and "keep" not in metadata.tags
                ):
            ...
            if len(self.snapshots_metadata) > self.max_snapshots:
                ...
                sorted_snapshots = sorted(
                    self.snapshots_metadata.items(), key=lambda x: x[1].timestamp
                )

                excess_count = len(sorted_snapshots) - self.max_snapshots
                for snapshot_id, _ in sorted_snapshots[:excess_count]:
                    if (
                        snapshot_id not in to_delete
                        and "keep" not in self.snapshots_metadata[snapshot_id].tags
                    ):
                        to_delete.append(snapshot_id)
    ...
    async def _validate_snapshot_integrity(
        self, metadata: SnapshotMetadata, snapshot_file: Path
    ) -> bool:
        ...
        try:
            ...
            if actual_size != metadata.size_bytes:
                self.logger.warning(
                    f"Size mismatch for snapshot {metadata.snapshot_id}: expected {metadata.size_bytes}, got {actual_size}"
                )
                return False

            # Check checksum
            actual_checksum = await self.integrity_checker.calculate_checksum(
                snapshot_file
            )
            if actual_checksum != metadata.checksum:
                self.logger.warning(
                    f"Checksum mismatch for snapshot {metadata.snapshot_id}"
                )
                return False
    ...
    async def _create_rollback_plan(
        self, snapshot_id: str, rollback_type: str
    ) -> RollbackPlan:
        ...
        if rollback_type == "full":
            ...
            affected_paths = [self.config.workspace_path / "state"]
        else:  # selective
    ...
    async def _execute_rollback(self, snapshot_file: Path, plan: RollbackPlan) -> int:
        ...
        try:
            # Extract archive safely using SecureTarExtractor
            ...
            for affected_path in plan.affected_paths:
                ...
                if source_path.exists():
                    ...
                    if source_path.is_file():
                        ...
                        files_restored += sum(
                            1 for _ in affected_path.rglob("*") if _.is_file()
                        )
