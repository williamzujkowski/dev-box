...
from ..utils.stub_modules import SecurityAnalyzer, PatternMatcher, RiskAssessment
...
class SafetyValidator:
    ...
    async def validate_operation(self, operation: Dict[str, Any]) -> ValidationResult:
        ...
        try:
            ...
            constraints = self.operation_constraints.get(
                operation_type,
                self.operation_constraints.get("default", OperationConstraints("default"))
            )
            ...
            result.suggestions = await self._generate_safety_suggestions(operation, result)
            
            # Record validation
            await self._record_validation(operation, result)
            
            self.logger.debug(f"Validation result: safe={result.is_safe}, risk={result.risk_level.value}")
            return result
    ...
    async def validate_content(self, content: str, content_type: str = "text") -> ValidationResult:
        """
        ...
        try:
            ...
            dangerous_patterns = await self._check_dangerous_patterns(content, content_type, result)
            
            # Check content size
            await self._validate_content_size(content, result)
            
            # Security analysis
            security_issues = await self.security_analyzer.analyze_content(content, content_type)
            if security_issues:
            ...
            return ValidationResult(
                is_safe=False,
                risk_level=RiskLevel.CRITICAL,
                violations=[f"Content validation error: {str(e)}"]
            )
    ...
    async def set_operation_constraints(self, operation_type: str, 
                                       constraints: OperationConstraints) -> bool:
        """
    ...
    async def get_validation_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            violations = sum(1 for v in self.recent_violations if not v.get("is_safe", True))
    ...
    async def _initialize_default_rules(self):
        ...
        try:
            ...
            await self.add_validation_rule(ValidationRule(
    ...
    async def _validate_content_safety(self, operation: Dict[str, Any], result: ValidationResult):
        ...
        try:
            ...
            result.violations.append(f"Content safety validation error: {str(e)}")
    
    async def _validate_resource_limits(self, operation: Dict[str, Any], 
                                      constraints: OperationConstraints, result: ValidationResult):
        ...
        try:
            ...
            result.violations.append(f"Resource limit validation error: {str(e)}")
    
    async def _validate_file_access(self, operation: Dict[str, Any], 
                                  constraints: OperationConstraints, result: ValidationResult):
        ...
        try:
            ...
            for file_path in file_paths:
                ...
                if path.suffix:
                    if (constraints.blocked_file_extensions and 
                        path.suffix.lower() in constraints.blocked_file_extensions):
                        result.violations.append(f"Blocked file extension: {path.suffix}")
                    
                    if (constraints.allowed_file_extensions and 
                        path.suffix.lower() not in constraints.allowed_file_extensions):
                        result.violations.append(f"File extension not allowed: {path.suffix}")
                
                # Check if path is within sandbox
                try:
                    workspace = self.config.workspace_path
                    if not str(path.resolve()).startswith(str(workspace.resolve())):
                        result.violations.append(f"File access outside sandbox: {file_path}")
                except:
                    result.violations.append(f"Invalid file path: {file_path}")
                    
        except Exception as e:
            result.violations.append(f"File access validation error: {str(e)}")
    
    async def _validate_command_execution(self, operation: Dict[str, Any], 
                                        constraints: OperationConstraints, result: ValidationResult):
        ...
        try:
            ...
            for command in commands:
                ...
                if constraints.blocked_commands:
                    ...
                    if command_parts and command_parts[0] in constraints.blocked_commands:
                ...
                if constraints.allowed_commands:
                    command_parts = command_str.split()
                    if command_parts and command_parts[0] not in constraints.allowed_commands:
                        result.violations.append(f"Command not in allowed list: {command_parts[0]}")
                        
        except Exception as e:
            result.violations.append(f"Command validation error: {str(e)}")
    
    async def _validate_network_access(self, operation: Dict[str, Any], 
                                     constraints: OperationConstraints, result: ValidationResult):
        ...
        try:
            ...
            network_allowed = self.safety_constraints.get("network", {}).get("allow_external", False)
            ...
            result.violations.append(f"Network access validation error: {str(e)}")
    
    async def _validate_security_patterns(self, operation: Dict[str, Any], result: ValidationResult):
        ...
        try:
            ...
            result.violations.append(f"Security pattern validation error: {str(e)}")
    
    async def _check_dangerous_patterns(self, content: str, content_type: str, 
        ...
        try:
            for rule in self.validation_rules.values():
                ...
                if rule.pattern and re.search(rule.pattern, content, re.IGNORECASE):
                    violation_msg = f"Dangerous pattern detected ({rule.name}): {rule.description}"
                    ...
                    elif rule.risk_level.value == "high" and result.risk_level != RiskLevel.CRITICAL:
                        result.risk_level = RiskLevel.HIGH
                        
        except Exception as e:
            result.violations.append(f"Pattern checking error: {str(e)}")
    ...
    async def _validate_content_size(self, content: str, result: ValidationResult):
        ...
        try:
            ...
            max_size = self.safety_constraints.get("max_content_size_mb", 10) * 1024 * 1024
            
            if content_size > max_size:
                result.violations.append(f"Content size exceeds limit: {content_size} > {max_size} bytes")
                
        except Exception as e:
            result.violations.append(f"Content size validation error: {str(e)}")
        ...
        try:
            ...
            for line in lines:
                ...
                if line and not line.startswith('#'):
                    ...
                    commands.extend([cmd.strip() for cmd in sub_commands if cmd.strip()])
    ...
    async def _generate_safety_suggestions(self, operation: Dict[str, Any], 
                                         result: ValidationResult) -> List[str]:
            ...
            if result.violations:
                ...
                for violation in result.violations:
                    if "command" in violation.lower():
                        suggestions.append("Consider using safer command alternatives or restrict command scope")
                    elif "file" in violation.lower():
                        suggestions.append("Limit file operations to sandbox workspace directory")
                    elif "network" in violation.lower():
                        suggestions.append("Disable network access or use local resources only")
                    elif "memory" in violation.lower() or "disk" in violation.lower():
                        suggestions.append("Reduce resource requirements or increase limits")
            
            if result.risk_level in [RiskLevel.HIGH, RiskLevel.CRITICAL]:
                suggestions.append("Consider creating a snapshot before proceeding")
                suggestions.append("Review operation parameters for potential security issues")
                suggestions.append("Test operation in a more restricted environment first")
    ...
    async def _record_validation(self, operation: Dict[str, Any], result: ValidationResult):
        ...
        try:
            ...
            if len(self.recent_violations) > self.max_violation_history:
                self.recent_violations = self.recent_violations[-self.max_violation_history:]
