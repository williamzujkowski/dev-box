...
class SandboxCore:
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.info(f"Sandbox {self.config.sandbox_id} initialized successfully")
    ...
    async def execute_operation(self, operation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if not safety_result.is_safe:
                raise Exception(f"Operation failed safety check: {safety_result.reason}")
            
            # Create pre-execution snapshot if needed
            snapshot_id = None
            if operation.get("create_snapshot", True):
                snapshot_id = await self.rollback_manager.create_snapshot(f"pre-{operation_id}")
        ...
        while not self._shutdown_event.is_set():
            try:
                ...
                await asyncio.wait_for(
                    ...
                    timeout=self.config.snapshot_frequency
                )
