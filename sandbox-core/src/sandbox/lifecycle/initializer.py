...
from ..utils.filesystem import safe_chmod
from ..utils.filesystem import safe_mkdir
...
class SandboxInitializer:
    ...
    async def setup_workspace(self) -> bool:
        ...
        try:
            ...
            if not validate_path(self.config.workspace_path):
                raise ValueError(
                    f"Invalid workspace path: {self.config.workspace_path}"
                )
        ...
        try:
            ...
            if not workspace_check["accessible"]:
                result.errors.append(
                    f"Workspace not accessible: {workspace_check['error']}"
                )
    ...
    async def _terminate_workspace_processes(self):
        ...
        try:
            ...
            self.logger.warning(
                "psutil not available - cannot terminate workspace processes"
            )
        except Exception as e:
