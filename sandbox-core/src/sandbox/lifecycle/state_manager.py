...
class StateManager:
    ...
    def __init__(self, config):
        ...
        self.encryption = StateEncryption() if config.safety_constraints.get("encrypt_state") else None
    ...
    async def set_state(self, key: str, value: Any, encrypted: bool = False, 
    ...
    async def get_state(self, key: str, version: Optional[int] = None, 
        ...
        try:
            async with self._lock:
                ...
                if pattern:
                    ...
                    cursor.execute("SELECT DISTINCT key FROM state_entries ORDER BY key")
    ...
    async def create_snapshot(self, snapshot_id: str, 
                            keys: Optional[List[str]] = None) -> Optional[StateSnapshot]:
        """
        ...
        try:
            async with self._lock:
                ...
                self.logger.info(f"Created state snapshot: {snapshot_id} ({len(entries)} entries)")
                return snapshot
    ...
    async def restore_snapshot(self, snapshot_id: str) -> bool:
        ...
        try:
            async with self._lock:
                ...
                self.logger.info(f"Restored {restored_count} state entries from snapshot: {snapshot_id}")
    ...
    async def cleanup_old_data(self, days: int = 7) -> int:
        ...
        try:
            async with self._lock:
                ...
                self.logger.info(f"Cleaned up {deleted_entries} state entries and {deleted_snapshots} snapshots")
    ...
    async def _get_state_entry(self, key: str, version: Optional[int] = None) -> Optional[StateEntry]:
    ...
    async def _load_initial_state(self):
        ...
        await self.set_state("config.safety_constraints", self.config.safety_constraints)
