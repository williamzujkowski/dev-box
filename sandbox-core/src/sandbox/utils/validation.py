...
class ConfigValidator:
    ...
    async def validate(self, config) -> ValidationResult:
        ...
        try:
            ...
            result.add_error("validation", f"Configuration validation failed: {str(e)}")
        
        return result
    
    async def validate_resource_limits(self, resource_limits: Dict[str, Any]) -> ValidationResult:
        """
        ...
        try:
            ...
            if memory_mb is not None:
                if not isinstance(memory_mb, (int, float)) or memory_mb <= 0:
                    result.add_error("memory_mb", "Must be a positive number", memory_mb)
                elif memory_mb < 64:
                    result.add_warning("memory_mb", "Very low memory limit may cause issues", memory_mb)
                elif memory_mb > 8192:
            ...
            if cpu_percent is not None:
                if not isinstance(cpu_percent, (int, float)) or cpu_percent <= 0 or cpu_percent > 100:
                    result.add_error("cpu_percent", "Must be between 0 and 100", cpu_percent)
                elif cpu_percent > 90:
                    result.add_warning("cpu_percent", "High CPU limit may affect system performance", cpu_percent)
            ...
            if disk_mb is not None:
                if not isinstance(disk_mb, (int, float)) or disk_mb <= 0:
                    ...
                    result.add_warning("disk_mb", "Very low disk limit may cause issues", disk_mb)
            ...
            result.add_error("resource_limits", f"Validation failed: {str(e)}")
        
        return result
    
    async def validate_safety_constraints(self, safety_constraints: Dict[str, Any]) -> ValidationResult:
        """
        ...
        try:
            ...
            if max_snapshots is not None:
                if not isinstance(max_snapshots, int) or max_snapshots < 1:
                    result.add_error("max_snapshots", "Must be a positive integer", max_snapshots)
                elif max_snapshots > 100:
                    result.add_warning("max_snapshots", "High snapshot count may use excessive disk space", max_snapshots)
            
            # Auto cleanup settings
            auto_cleanup_hours = safety_constraints.get("auto_cleanup_hours")
            if auto_cleanup_hours is not None:
                if not isinstance(auto_cleanup_hours, (int, float)) or auto_cleanup_hours < 1:
                    result.add_error("auto_cleanup_hours", "Must be at least 1 hour", auto_cleanup_hours)
                elif auto_cleanup_hours < 24:
                    result.add_warning("auto_cleanup_hours", "Short cleanup interval may delete recent snapshots", auto_cleanup_hours)
            ...
            if network_config:
                ...
                if allow_external is not None and not isinstance(allow_external, bool):
                    result.add_error("network.allow_external", "Must be a boolean value", allow_external)
            
            # Encryption settings
            encrypt_state = safety_constraints.get("encrypt_state")
            if encrypt_state is not None and not isinstance(encrypt_state, bool):
                result.add_error("encrypt_state", "Must be a boolean value", encrypt_state)
            
            encrypt_snapshots = safety_constraints.get("encrypt_snapshots")
            if encrypt_snapshots is not None and not isinstance(encrypt_snapshots, bool):
                result.add_error("encrypt_snapshots", "Must be a boolean value", encrypt_snapshots)
            
        except Exception as e:
            result.add_error("safety_constraints", f"Validation failed: {str(e)}")
    ...
    async def _validate_sandbox_id(self, sandbox_id: str, result: ValidationResult):
        ...
        if not re.match(r'^[a-zA-Z0-9_-]+$', sandbox_id):
            result.add_error("sandbox_id", "Can only contain letters, numbers, hyphens, and underscores", sandbox_id)
        
        # Check length
        if len(sandbox_id) < 3:
            result.add_error("sandbox_id", "Must be at least 3 characters long", sandbox_id)
        elif len(sandbox_id) > 64:
            result.add_error("sandbox_id", "Must be no more than 64 characters long", sandbox_id)
    ...
    async def _validate_workspace_path(self, workspace_path: Path, result: ValidationResult):
        """Validate workspace path"""
        if not isinstance(workspace_path, Path):
            try:
                workspace_path = Path(workspace_path)
            except Exception:
                result.add_error("workspace_path", "Must be a valid path", workspace_path)
                return
        
        # Check if path is absolute
        if not workspace_path.is_absolute():
            result.add_warning("workspace_path", "Relative paths may cause issues", str(workspace_path))
        ...
        for dangerous_path in dangerous_paths:
            if workspace_str.startswith(dangerous_path) and workspace_str != dangerous_path + "/sandbox":
                result.add_error("workspace_path", f"Cannot use system path: {dangerous_path}", workspace_str)
        
        # Check parent directory accessibility
        try:
            parent = workspace_path.parent
            if parent.exists() and not parent.is_dir():
                result.add_error("workspace_path", "Parent is not a directory", str(parent))
        except Exception as e:
            result.add_warning("workspace_path", f"Cannot validate parent directory: {str(e)}")
    
    async def _validate_resource_limits(self, resource_limits: Dict[str, Any], result: ValidationResult):
        """Validate resource limits configuration"""
        if not isinstance(resource_limits, dict):
            result.add_error("resource_limits", "Must be a dictionary", type(resource_limits))
            return
    ...
    async def _validate_safety_constraints(self, safety_constraints: Dict[str, Any], result: ValidationResult):
        """Validate safety constraints configuration"""
        if not isinstance(safety_constraints, dict):
            result.add_error("safety_constraints", "Must be a dictionary", type(safety_constraints))
            return
    ...
    async def _validate_monitoring_config(self, monitoring_config: Dict[str, Any], result: ValidationResult):
        """Validate monitoring configuration"""
        if not isinstance(monitoring_config, dict):
            result.add_error("monitoring_config", "Must be a dictionary", type(monitoring_config))
            return
        
        # Collection interval
        collection_interval = monitoring_config.get("collection_interval")
        if collection_interval is not None:
            if not isinstance(collection_interval, (int, float)) or collection_interval <= 0:
                result.add_error("monitoring_config.collection_interval", "Must be a positive number", collection_interval)
            elif collection_interval < 1:
                result.add_warning("monitoring_config.collection_interval", "Very frequent collection may impact performance", collection_interval)
        ...
        if health_check_interval is not None:
            if not isinstance(health_check_interval, (int, float)) or health_check_interval <= 0:
                result.add_error("monitoring_config.health_check_interval", "Must be a positive number", health_check_interval)
    ...
    async def _validate_resource_thresholds(self, thresholds: Dict[str, Any], result: ValidationResult):
        ...
        for field, (min_val, max_val) in threshold_fields.items():
            ...
            if value is not None:
                if not isinstance(value, (int, float)):
                    result.add_error(f"resource_thresholds.{field}", "Must be a number", value)
                elif value < min_val:
                    result.add_error(f"resource_thresholds.{field}", f"Must be at least {min_val}", value)
                elif max_val is not None and value > max_val:
                    result.add_error(f"resource_thresholds.{field}", f"Must be at most {max_val}", value)
    
    async def _validate_network_limits(self, network_config: Dict[str, Any], result: ValidationResult):
        ...
        if max_bandwidth_mbps is not None:
            if not isinstance(max_bandwidth_mbps, (int, float)) or max_bandwidth_mbps <= 0:
                result.add_error("network.max_bandwidth_mbps", "Must be a positive number", max_bandwidth_mbps)
        
        # Connection limits
        max_connections = network_config.get("max_connections")
        if max_connections is not None:
            if not isinstance(max_connections, int) or max_connections <= 0:
                result.add_error("network.max_connections", "Must be a positive integer", max_connections)
    ...
    async def _validate_consistency(self, config, result: ValidationResult):
        ...
        try:
            ...
            result.add_warning("consistency", f"Could not validate consistency: {str(e)}")
