...
from ..utils.stub_modules import EventEmitter
from ..utils.stub_modules import MetricsCollector
...
class StateTracker:
    ...
    def __init__(self, config):
        ...
        # Configuration
        self.collection_interval = config.monitoring_config.get(
            "collection_interval", 30
        )  # seconds
        self.max_operation_history = config.monitoring_config.get(
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            await self.event_emitter.emit(
                "tracker_started",
                {"sandbox_id": self.config.sandbox_id, "start_time": self.start_time},
            )
    ...
    async def track_operation_complete(self, operation_id: str, result: Any):
        ...
        try:
            if operation_id not in self.operations:
                self.logger.warning(
                    f"Operation {operation_id} not found for completion tracking"
                )
                return
            ...
            self.logger.error(
                f"Failed to track operation completion {operation_id}: {e}"
            )
    ...
    async def track_operation_failed(self, operation_id: str, error: str):
        ...
        try:
            if operation_id not in self.operations:
                self.logger.warning(
                    f"Operation {operation_id} not found for failure tracking"
                )
                return
    ...
    async def get_recent_operations(self, limit: int = 10) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            sorted_ops = sorted(
                self.operations.values(), key=lambda op: op.start_time, reverse=True
            )
    ...
    async def get_performance_metrics(
        ...
        try:
            ...
            recent_operations = [
                op for op in self.operations.values() if op.start_time >= window_start
            ]
            ...
            successful_ops = [
                op for op in recent_operations if op.status == "completed"
            ]
    ...
    async def _capture_resource_snapshot(self) -> ResourceSnapshot:
        ...
        try:
            ...
            disk_usage = (
                psutil.disk_usage(str(self.config.workspace_path))
            ...
            return ResourceSnapshot(
                timestamp=time.time(), cpu_percent=0, memory_mb=0, disk_usage_mb=0
            )
