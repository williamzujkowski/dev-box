...
from ..utils.stub_modules import NetworkChecker, ProcessMonitor
...
class HealthReport:
    ...
    def severity(self) -> str:
        if self.overall_status == HealthStatus.CRITICAL:
            return "critical"
        elif self.overall_status == HealthStatus.WARNING:
            return "warning"
        else:
            return "normal"
...
class HealthMonitor:
    ...
    def __init__(self, config):
        ...
        # Configuration
        self.check_interval = config.monitoring_config.get("health_check_interval", 60)  # seconds
        self.critical_thresholds = config.monitoring_config.get("critical_thresholds", {})
        self.warning_thresholds = config.monitoring_config.get("warning_thresholds", {})
        self.enabled_checks = set(config.monitoring_config.get("enabled_health_checks", [
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if not initial_report.is_healthy:
                self.logger.warning(f"Initial health check failed: {initial_report.issues}")
    ...
    async def check_health(self) -> HealthReport:
        ...
        try:
            ...
            self.logger.debug(f"Health check completed in {check_duration:.2f}s - Status: {overall_status.value}")
        ...
        try:
            if self.last_health_report:
                return {
                    ...
                    "checks_passed": len([c for c in self.last_health_report.checks if c.status == HealthStatus.HEALTHY]),
            ...
            while not self._stop_event.is_set():
                try:
                    if not self.is_paused:
                        ...
                        if report.overall_status == HealthStatus.CRITICAL and self.auto_recovery:
    ...
    async def _check_resource_usage(self) -> HealthCheck:
        ...
        try:
            ...
            # Determine status
            if (cpu_percent >= critical_cpu or memory_percent >= critical_memory or 
                disk_percent >= critical_disk):
                status = HealthStatus.CRITICAL
                message = f"Critical resource usage: CPU {cpu_percent:.1f}%, Memory {memory_percent:.1f}%, Disk {disk_percent:.1f}%"
            elif (cpu_percent >= warning_cpu or memory_percent >= warning_memory or 
                  disk_percent >= warning_disk):
    ...
    async def _check_filesystem_health(self) -> HealthCheck:
        ...
        try:
            ...
            is_accessible = await self.filesystem_checker.check_accessibility(workspace_path)
            ...
            return HealthCheck(
                name="filesystem",
                status=HealthStatus.UNKNOWN,
                message=f"Failed to check filesystem: {str(e)}",
                details={"error": str(e)},
                check_duration=time.time() - start_time
            )
    ...
    async def _check_network_health(self) -> HealthCheck:
        ...
        try:
            ...
            network_required = self.config.safety_constraints.get("network", {}).get("required", False)
            ...
            if network_required and not network_status.get("reachable", False):
                ...
                status = HealthStatus.WARNING
                message = f"High network latency: {network_status.get('latency_ms', 0)}ms"
            else:
                status = HealthStatus.HEALTHY
    ...
    async def _check_application_health(self) -> HealthCheck:
        ...
        try:
            ...
            return HealthCheck(
                name="application",
                status=HealthStatus.UNKNOWN,
                message=f"Failed to check application health: {str(e)}",
                details={"error": str(e)},
                check_duration=time.time() - start_time
            )
    ...
    async def _attempt_recovery(self, report: HealthReport):
        ...
        try:
            ...
            self.logger.info(f"Recovery attempt completed: {success_count}/{len(recovery_actions)} actions successful")
