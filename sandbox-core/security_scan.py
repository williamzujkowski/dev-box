    ...
    def run_full_scan(self) -> Dict[str, Any]:
        ...
        for scan_name, scan_result in results["scans"].items():
            if scan_result.get("success"):
                total_issues += scan_result.get("issues_found", 0) + scan_result.get("vulnerabilities_found", 0)
        ...
        results["summary"] = {
            ...
            "security_score": max(0, 100 - (total_high * 20) - (total_medium * 5) - (total_low * 1))
        }
        
        return results
    
    def generate_report(self, results: Dict[str, Any], output_file: Path = None) -> None:
        ...
        for scan_name, scan_result in results.get("scans", {}).items():
            ...
            if scan_result.get("success"):
                if scan_name == "patterns":
                    ...
                    print(f"  Vulnerabilities: {scan_result.get('vulnerabilities_found', 0)}")
...
def main():
    ...
    parser.add_argument("--fail-on-medium", action="store_true",
                       help="Fail on medium severity issues")
