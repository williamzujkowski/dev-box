...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "requires_vm: mark test as requiring actual VM")
...
def mock_subprocess():
    ...
    mock = Mock()
    mock.run.return_value = Mock(returncode=0, stdout="Mock command output", stderr="")
    return mock
...
def security_test_commands():
    ...
    return {
        "safe_commands": ["ls -la", "pwd", "whoami", "npm install", "node --version"],
        "dangerous_commands": [
            "rm -rf /",
