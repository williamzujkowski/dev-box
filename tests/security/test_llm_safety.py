...
class TestCommandValidation:
    ...
    def test_suspicious_command_flagged(self, security_test_commands):
        ...
        for command in suspicious_commands:
            ...
            assert requires_confirmation, f"Suspicious command '{command}' should require confirmation"
...
class TestSandboxIsolation:
    ...
    def test_network_isolation(self, test_vm_manager):
        ...
        network_tests = [
            ...
            {"target": "10.0.0.1", "port": 443, "should_block": True},  # Internal network
            {"target": "github.com", "port": 443, "should_block": False}  # Allowed external
        ]
        
        for test in network_tests:
            access_result = self._test_network_access(vm_id, test["target"], test["port"])
            if test["should_block"]:
                assert not access_result, f"Access to {test['target']}:{test['port']} should be blocked"
            else:
                assert access_result, f"Access to {test['target']}:{test['port']} should be allowed"
...
class TestSecurityPolicyEnforcement:
    ...
    def test_command_restriction_enforcement(self, test_vm_manager):
        ...
        forbidden_commands = self.security_policies["command_restrictions"]["forbidden_commands"]
    ...
    def test_path_restriction_enforcement(self, test_vm_manager):
        ...
        restricted_paths = self.security_policies["command_restrictions"]["restricted_paths"]
        ...
        for path in restricted_paths:
            ...
            for command in test_commands:
                ...
                assert path_access_blocked, f"Access to restricted path '{path}' should be blocked"
    ...
    def test_network_policy_enforcement(self, test_vm_manager):
        ...
        for domain in allowed_domains:
            ...
            assert access_allowed, f"Access to allowed domain '{domain}' should be permitted"
        ...
        for domain in blocked_domains:
            ...
            assert access_blocked, f"Access to blocked domain '{domain}' should be denied"
    ...
    def test_resource_policy_enforcement(self, test_vm_manager):
        ...
        memory_limit_enforced = self._test_memory_policy_enforcement(vm_id, memory_limit)
        assert memory_limit_enforced
        ...
        process_limit_enforced = self._test_process_limit_enforcement(vm_id, process_limit)
    ...
    def test_policy_violation_reporting(self, test_vm_manager, mock_logger):
        ...
        for violation in violations:
            ...
            assert violation_logged, f"Policy violation of type '{violation['type']}' should be logged"
    ...
    def _attempt_policy_violation(self, vm_id, violation, logger):
        ...
        logger.warning(f"Policy violation detected: {violation['type']} - {violation['action']}")
