...
class TestRunner:
    ...
    def run_smoke_tests(self, verbose=False):
        ...
        cmd = [
            "python", "-m", "pytest",
            "tests/",
            "-k", "test_basic or test_smoke or test_sanity",
            "--timeout=30"
        ]
...
Examples:
  ...
  python run_tests.py pattern "vm_*"    # Run tests matching pattern
    ...
    parser.add_argument(
        "pattern",
        nargs="?",
        help="Test pattern (for 'pattern' test type)"
    )
    ...
    if args.test_type == "unit":
        ...
        success = runner.run_all_tests(args.verbose, not args.no_coverage, args.skip_slow)
